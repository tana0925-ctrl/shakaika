...
app.get('/api/admin/annual-notes', authMiddleware, adminMiddleware, async (c) => {
  const qid = c.req.query('user_id') || c.req.query('userId') || ''
  const userId = parseInt(qid)
  const qfy = c.req.query('fy')
  const fy = qfy ? parseInt(qfy) : getCurrentFiscalYear()
  const fiscalYear = Number.isFinite(fy) ? fy : getCurrentFiscalYear()
...
async function ensureUser() {
  ...
  try {
    const res = await fetchWithTimeout('/api/auth/me', { headers: { 'Authorization': 'Bearer ' + token } }, 12000);
...
async function saveSelections() {
  ...
  try {
    ...
    for (const res of responses) {
      if (!res || !res.ok) {
        let data = null;
        try { data = res ? await res.json() : null; } catch(e) {}
        const msg = (data && data.error) ? data.error : ('保存に失敗しました（' + (res ? res.status : '0') + '）');
        throw new Error(msg);
      }
    }
...
async function init() {
  ...
  if (userName) userName.textContent = me.name + ' さん' + (me.school ? '（' + me.school + '）' : '');
...
async function ensureUser() {
  ...
  try {
    ...
    if (!res.ok) return null;
    const data = await res.json();
    if (data && data.user) { user = data.user; localStorage.setItem('user', JSON.stringify(user)); return user; }
  } catch(e) {}
  return null;
}
...
async function initAdmin() {
  const me = await ensureUser();
  if (!token || !me || me.role !== 'admin') { window.location.href = '/login'; return false; }
  user = me;
  const adminName = document.getElementById('adminName');
  if (adminName) adminName.textContent = me.name + ' さん';
  return true;
}
...
function buildFiscalYearOptions(currentFY) {
  // 直近5年分を表示（必要ならここを増やせます）
  const years = [];
  for (let i = 0; i < 5; i++) years.push(currentFY - i);
  return years;
}

async function loadAnnualNotesForMember(userId, fy) {
...
html += '<div style="margin:18px 0 18px;padding:14px;border:2px solid #f0f0f0;border-radius:12px;background:#fafafa">' +
    '<div style="display:flex;justify-content:space-between;align-items:center;gap:10px;margin-bottom:10px">' +
      '<div style="font-weight:700;color:#1a237e"><i class="fas fa-bullseye"></i> 年度の目標・振り返り</div>' +
      '<div style="display:flex;align-items:center;gap:6px">' +
        '<span style="font-size:12px;color:#666">年度</span>' +
        '<select id="annualFySelect" style="padding:6px 8px;border:1px solid #ddd;border-radius:8px;font-family:inherit"></select>' +
      '</div>' +
    '</div>' +
  ...
  // 年度の目標・振り返り（管理者閲覧）
  const fySelect = document.getElementById('annualFySelect');
  if (fySelect) {
    const currentFY = getCurrentFiscalYearClient();
    const years = buildFiscalYearOptions(currentFY);
    fySelect.innerHTML = years.map(y => '<option value="' + y + '">' + y + '年度</option>').join('');
    fySelect.value = String(currentFY);
    fySelect.addEventListener('change', function() {
      const fy = parseInt(fySelect.value);
      loadAnnualNotesForMember(id, fy);
    });
    loadAnnualNotesForMember(id, currentFY);
  }
}
...
initAdmin().then((ok)=>{ if(ok) loadMembers(); });
</script>
</body></html>`)
})
...
initEvents().then((ok)=>{ if(ok) loadEvents(); });
</script>
</body></html>`)
})
...
// --- Root (smart redirect) ---
app.get('/', (c) => {
  // Can't read localStorage on the server, so return a tiny landing page that
  // checks auth on the client and redirects accordingly.
  return c.html(`<!DOCTYPE html><html lang="ja"><head>${commonHead}
<title>社会科同好会</title>
<style>
  body{font-family:system-ui,-apple-system,'Segoe UI',Roboto,'Zen Maru Gothic',sans-serif;background:#fafafa;color:#333;margin:0;padding:24px}
  .card{max-width:720px;margin:0 auto;background:#fff;border-radius:14px;padding:18px 18px 12px;box-shadow:0 2px 12px rgba(0,0,0,.06)}
  ...
  <div class="card">
    ...
    <div class="muted" id="msg">ログイン状態を確認しています。</div>
  </div>
<script>
(function(){
  ...
  fetch('/api/auth/me', { headers: { 'Authorization': 'Bearer ' + token }})
    .then(async (res) => {
      if (res.status === 401) { localStorage.clear(); location.replace('/login'); return; }
      if (!res.ok) throw new Error('認証確認に失敗しました（' + res.status + '）');
      const data = await res.json();
      if (!data || !data.user) throw new Error('ユーザー情報が取得できません');
      localStorage.setItem('user', JSON.stringify(data.user));
      if (data.user.role === 'admin') location.replace('/admin');
      else location.replace('/mypage');
    })
    .catch((e) => {
      console.error(e);
      if (msg) msg.innerHTML = '読み込みに失敗しました。<br><a href="/login">ログインへ</a>';
    });
})();
</script>
</body></html>`)
